...
export default function PatientsDetails() {
  ...
  useEffect(() => {
    if (!id) return;
    patientAPI.get(id).then(setPatient).catch((requestError: unknown) => setError(requestError instanceof Error ? requestError.message : "Patient not found"));
  }, [id]);
  ...
  if (!patient) return <BaseLayout resourceName="Patient Details"><div className="rounded-2xl bg-blue-50 p-6 text-sm">{error || "Loading patient record..."}</div></BaseLayout>;
  ...
  return <BaseLayout resourceName="Patient Details">
    <div className="rounded-2xl bg-blue-50 p-6"><div className="mx-auto max-w-4xl">{error && <div className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</div>}<button onClick={() => navigate("/patients")} className="mb-4 text-sm font-semibold text-blue-600 hover:text-blue-700">← Back to Patient Management</button><div className="rounded-2xl bg-white p-8 shadow-sm">
